def m_write_playlist(p_playlist_name)
  ...
  CSV.open("/media/nollenr-r7e64/playlists/"+p_playlist_name+".txt", "w", {col_sep: "\cI"}) do |f_csv|
    ...
    MasterLibraryFile.joins(playlist_songs: :playlist).where(playlists: {name: p_playlist_name}).order("playlist_songs.playlist_order").each do |myrec|
      f_csv << [myrec.title,          myrec.artist,         v_composer,
                myrec.album,          v_grouping,           myrec.genre,
                v_size,               v_time,               v_disc_number,
                v_disc_count,         v_track_number,       v_track_count,
                myrec.year,           v_date_modified,      v_date_added,
                myrec.bitrate,        myrec.sample_rate,    v_volume_adjustment,
                v_kind,               v_equalizer,          v_comments,
                v_plays,              v_last_played,        v_skips,
                v_last_skipped,       v_my_rating,          m_windows_fqfn(myrec.master_directory_location)]
    end
  end
  return true
end
